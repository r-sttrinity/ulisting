...
<div id="stm-listing-form-listing">
                ...
                <div v-for="plan in user_plans" class="stm-col-4" >
                    <div class="card" v-if="plan.type != 'feature' && (plan.listing_limit || plan.listing_limit == 0) || (listing_plan_select == 'none') && (plan.id == 'free' || plan.id == listing_plan_select) && plan.status === 'active'">
                        <div class="card-body">
                            ...
                            <button v-if="!listing_plan_one_time" :disabled="plan.listing_limit <= plan.use_listing_limit" @click="select_limit_plan(plan)" class="btn btn-primary"><?php echo __('select', 'ulisting');?></button>
                        </div>
                    </div>
                </div>
            </div>
            <div class="stm-row">
                <template v-if="!isAdmin">
                    <div class="col text-center uListing-buy-plan">
                        <a class="btn btn-primary" href="<?php echo esc_url(ulisting_get_page_link( 'pricing_plan' )); ?>"><?php _e('Buy plan', "ulisting")?> </a>
                    </div>
                </template>
                <template v-else>
    ...
    <div class="step"  v-if="step == 'last'">
        ...
        <div>
            ...
            <a href="<?php echo esc_url(\uListing\Classes\StmUser::getUrl('my-listing')); ?>" class="btn btn-default">
                <?php _e("My listing")?>
            </a>
        </div>
    </div>
