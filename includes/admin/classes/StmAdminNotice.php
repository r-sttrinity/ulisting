...
class StmAdminNotice
{
    ...
    {
		add_action('admin_notices', [$this, 'render_review']);
	...
	public function render_review()
	{
		...
		$installed_time 	= get_option('ulisting_installed');
		$canceled_rating 	= !empty(get_option('ulisting_canceled')) ? get_option('ulisting_canceled') : 'no';
		...
		if ($interval_value >= 7 && $canceled_rating === 'no') {
			wp_enqueue_script('ulisting-notice-review', ULISTING_URL .'/assets/js/stm-review-notice.js', array(), false, true);

			ulisting_render_template(ULISTING_ADMIN_PATH . '/views/notices/review.php', [], true);
		}
	}
