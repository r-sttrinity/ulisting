...
function ulisting_add_nonces()
{
    $uListing_api_url = site_url()."/1/api";

    $variables = array(
        'ulisting_ajax_add_review'  => wp_create_nonce( 'ulisting_ajax_add_review' ),

    );
    echo( '<script type="text/javascript">window.data = ' . json_encode( $variables ) . '; var uListing_api_url = '. json_encode($uListing_api_url) .' </script>' );
}

function ulisting_ajax_add_review()
{
    $status = isset($_GET[ 'add_review_status' ]) ? sanitize_text_field($_GET[ 'add_review_status' ]) : '';
    update_option( 'ulisting_canceled', $status );
}

add_action( 'wp_head', 'ulisting_add_nonces' );
add_action( 'admin_head', 'ulisting_add_nonces' );
add_action( 'wp_ajax_ulisting_ajax_add_review', 'ulisting_ajax_add_review' );
add_action( 'wp_ajax_ulisting_ajax_add_review', 'ulisting_ajax_add_review' );
